...
try {
    ...
    try {
        ...
        $arguments = @('--source', $source, '--target', $target, '--main', 'NetCheckMonitor.exe', '--version', '0.9.14', '--wait-pid', '2147483000', '--wait-start', '0', '--health', $health, '--log', $log, '--manifest-digest', $manifestDigest, '--resume', '0')
    ...
    $invalidArguments = @('--source', $invalidSource, '--target', $invalidTarget, '--main', 'NetCheckMonitor.exe', '--version', '0.9.14', '--wait-pid', '2147483000', '--wait-start', '0', '--health', (Join-Path $testRoot 'invalid-health.txt'), '--log', $invalidLog, '--manifest-digest', $invalidManifestDigest, '--resume', '0')
