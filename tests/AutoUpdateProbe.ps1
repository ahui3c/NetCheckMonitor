...
try {
    ...
    $validated = $method.Invoke($null, [object[]]@([string]$packagePath, [string]$digest, [string]'v0.9.17', [string]'https://example.invalid/release', [string]$validRoot))
    $validPackage = $validated.Version -eq '0.9.17' -and
    ...
    try {
        ...
        [void]$method.Invoke($null, [object[]]@([string]$packagePath, [string]$badDigest, [string]'v0.9.17', [string]'', [string]$badRoot))
    ...
    try {
        ...
        [void]$method.Invoke($null, [object[]]@([string]$maliciousZip, [string]$maliciousDigest, [string]'v0.9.17', [string]'', [string]$zipRoot))
