...
try {
    ...
    $validated = $method.Invoke($null, [object[]]@([string]$packagePath, [string]$digest, [string]'v0.9.14', [string]'https://example.invalid/release', [string]$validRoot))
    $validPackage = $validated.Version -eq '0.9.14' -and
    ...
    try {
        ...
        [void]$method.Invoke($null, [object[]]@([string]$packagePath, [string]$badDigest, [string]'v0.9.14', [string]'', [string]$badRoot))
    ...
    try {
        ...
        [void]$method.Invoke($null, [object[]]@([string]$maliciousZip, [string]$maliciousDigest, [string]'v0.9.14', [string]'', [string]$zipRoot))
    ...
    if (-not ($validPackage -and $wrongDigestRejected -and $zipSlipRejected -and $escapedFileAbsent)) { throw 'Automatic update package validation failed.' }
} finally {
