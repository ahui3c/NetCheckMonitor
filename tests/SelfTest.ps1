...
$homeVersionLabel = $versionLabel.Text -eq 'v0.9.12' -and $versionLabel.Font.Size -le 8.5 -and $versionLabel.ForeColor -eq [Drawing.Color]::DarkGray
...
$versionComparison = $isNewerVersionMethod.Invoke($null, @('v0.9.13')) -and -not $isNewerVersionMethod.Invoke($null, @('v0.9.12')) -and -not $isNewerVersionMethod.Invoke($null, @('v0.9.11'))
$aboutText = @($aboutForm.Controls | ForEach-Object { $_.Text }) -join "`n"
$aboutPageContent = $aboutForm.Text -eq '關於 NetCheckMonitor' -and $aboutText.Contains('NetCheckMonitor') -and $aboutText.Contains('版本 0.9.12') -and $aboutText.Contains('可定時監控對外網路連線，紀錄斷線並產生圖文報表，並支援網路硬碟備份，PDF 下載，程式完全免費開源無廣告。') -and $aboutText.Contains('廖阿輝') -and $aboutText.Contains('chehui@gmail.com') -and $aboutText.Contains('https://ahui3c.com') -and $aboutText.Contains('https://github.com/ahui3c/NetCheckMonitor') -and $checkVersionButton.Text -eq '檢查新版本'
...
$programIdentity = $form.Text -eq '對外網路連線能力監控程式' -and $assembly.GetName().Version.ToString() -eq '0.9.12.0'
