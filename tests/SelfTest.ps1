...
$cloudBackupStatusPanel = $type.GetField('cloudBackupStatusPanel', $flags).GetValue($form)
$cloudBackupStatusTitle = $type.GetField('cloudBackupStatusTitle', $flags).GetValue($form)
$cloudBackupStatusDetail = $type.GetField('cloudBackupStatusDetail', $flags).GetValue($form)
$cloudBackupSettingsLink = $type.GetField('cloudBackupSettingsLink', $flags).GetValue($form)
$recentList = $type.GetField('recentList', $flags).GetValue($form)
$cloudStatusMethod = $type.GetMethod('BuildCloudBackupStatusText', [Reflection.BindingFlags]'Static,NonPublic')
$cloudDisconnectedText = @($cloudStatusMethod.Invoke($null, [object[]]@($false, $false, [TimeSpan]::Parse('23:55'), $false)))
$cloudConnectedText = @($cloudStatusMethod.Invoke($null, [object[]]@($true, $false, [TimeSpan]::Parse('21:30'), $true)))
$cloudBackingUpText = @($cloudStatusMethod.Invoke($null, [object[]]@($true, $true, [TimeSpan]::Parse('21:30'), $true)))
$homeCloudBackupStatus = $cloudBackupStatusTitle.Text.Contains('Google Drive 備份：未連接') -and
    $cloudBackupStatusDetail.Text.Contains('每日 PDF＋CSV：關閉') -and $cloudBackupStatusDetail.Text.Contains('定時測速資料備份：關閉') -and
    $cloudBackupSettingsLink.Text -eq '設定…' -and $cloudBackupStatusPanel.Top -gt $networkInfoLabel.Bottom -and $recentList.Top -gt $cloudBackupStatusPanel.Bottom -and
    $cloudDisconnectedText[0].Contains('未連接') -and $cloudDisconnectedText[1].Contains('尚未連接') -and
    $cloudConnectedText[0].Contains('已連接') -and $cloudConnectedText[1].Contains('每日 PDF＋CSV：開啟（21:30）') -and $cloudConnectedText[1].Contains('定時測速資料備份：開啟') -and
    $cloudBackingUpText[0].Contains('備份中')
...
$homeVersionLabel = $versionLabel.Text -eq 'v0.9.20' -and $versionLabel.Font.Size -le 8.5 -and $versionLabel.ForeColor -eq [Drawing.Color]::DarkGray
...
$settingsGmailButton = $settingsFormType.GetField('gmailSettingsButton', $flags).GetValue($settingsForm)
$settingsClearButton = $settingsFormType.GetField('clearDataButton', $flags).GetValue($settingsForm)
$settingsExportButton = $settingsFormType.GetField('exportBackupButton', $flags).GetValue($settingsForm)
...
$settingsCompactLayout = $settingsForm.ClientSize.Height -le 700 -and $settingsCustomRadio.Top -le 160 -and ($settingsSaveButton.Bottom + 12) -le $settingsForm.ClientSize.Height -and
    ...
    $settingsClearButton.Width -lt $settingsExportButton.Width -and $settingsRebuildButton.Width -lt $settingsExportButton.Width
...
$versionComparison = $isNewerVersionMethod.Invoke($null, @('v0.9.21')) -and -not $isNewerVersionMethod.Invoke($null, @('v0.9.20')) -and -not $isNewerVersionMethod.Invoke($null, @('v0.9.19'))
$aboutText = @($aboutForm.Controls | ForEach-Object { $_.Text }) -join "`n"
$aboutPageContent = $aboutForm.Text -eq '關於 NetCheckMonitor' -and $aboutText.Contains('NetCheckMonitor') -and $aboutText.Contains('版本 0.9.20') -and $aboutText.Contains('可定時監控對外網路連線，紀錄斷線並產生圖文報表，並支援網路硬碟備份，PDF 下載，程式完全免費開源無廣告。') -and $aboutText.Contains('廖阿輝') -and $aboutText.Contains('chehui@gmail.com') -and $aboutText.Contains('https://ahui3c.com') -and $aboutText.Contains('https://github.com/ahui3c/NetCheckMonitor') -and $checkVersionButton.Text -eq '線上更新'
...
$programIdentity = $form.Text -eq '對外網路連線能力監控程式' -and $assembly.GetName().Version.ToString() -eq '0.9.20.0'
...
$cloudComputerFolder = $cloudManagerType.GetMethod('RunComputerFolderSelfTest', [Reflection.BindingFlags]'Static,Public').Invoke($null, @())
...
$cloudFormText = @($cloudForm.Controls | ForEach-Object { $_.Text }) -join "`n"
$oauthLoginOnlyUi = $cloudConnectButton.Text -eq '登入 Google Drive' -and $cloudFormText.Contains('Drive / Net_Check / 電腦名稱') -and $null -eq $cloudFormType.GetMethod('Open', [Reflection.BindingFlags]'Static,NonPublic')
$gmailManagerType = $assembly.GetType('NetCheck.GmailNotificationManager', $true)
[string]$gmailStorageTest = Join-Path $testRoot 'Gmail\storage-selftest.dat'
$gmailStorageProtected = $gmailManagerType.GetMethod('RunStorageSelfTest', [Reflection.BindingFlags]'Static,Public').Invoke($null, [object[]]@($gmailStorageTest))
$gmailSelfRecipientMime = $gmailManagerType.GetMethod('RunMimeSelfTest', [Reflection.BindingFlags]'Static,Public').Invoke($null, @())
$gmailLargeReportPayload = $gmailManagerType.GetMethod('RunLargePayloadSelfTest', [Reflection.BindingFlags]'Static,Public').Invoke($null, @())
$gmailMachineNameSubject = $gmailManagerType.GetMethod('RunSubjectSelfTest', [Reflection.BindingFlags]'Static,Public').Invoke($null, @())
$gmailOAuthRequest = $gmailManagerType.GetMethod('RunOAuthRequestSelfTest', [Reflection.BindingFlags]'Static,Public').Invoke($null, @())
$gmailManager = [Activator]::CreateInstance($gmailManagerType, [object[]]@([Environment]::MachineName, 'A1B2C3D4'))
$gmailFormType = $assembly.GetType('NetCheck.GmailNotificationForm', $true)
$gmailForm = [Activator]::CreateInstance($gmailFormType, [object[]]@($gmailManager))
$gmailFormText = @($gmailForm.Controls | ForEach-Object { $_.Text }) -join "`n"
$gmailFixedSelfRecipientUi = $gmailForm.Text -eq 'Gmail 報表與恢復通知' -and $gmailFormText.Contains('寄件者與收件者固定為登入的同一個 Google 帳戶') -and $gmailFormText.Contains('每日寄送網路監控與定時測速報表（如有）') -and $gmailFormText.Contains('網路恢復後寄送通知') -and $gmailFormText.Contains('寄送測試郵件') -and @($gmailForm.Controls | Where-Object { $_ -is [Windows.Forms.TextBox] }).Count -eq 0
...
$applicationStartupType = $assembly.GetType('NetCheck.ApplicationStartup', $true)
$startupConfigureMethod = $applicationStartupType.GetMethod('Configure', [Reflection.BindingFlags]'Static,NonPublic')
$resumeWithoutPromptMethod = $applicationStartupType.GetMethod('ShouldResumeWithoutPrompt', [Reflection.BindingFlags]'Static,NonPublic')
$startupConfigureMethod.Invoke($null, [object[]]@(,[string[]]@('--windows-autostart')))
$unattendedResumeEnabled = $resumeWithoutPromptMethod.Invoke($null, [object[]]@($updatedSettings))
$updatedSettings.AutoStartMonitoring = $false
$unattendedResumeNeedsBothSettings = -not $resumeWithoutPromptMethod.Invoke($null, [object[]]@($updatedSettings))
$startupConfigureMethod.Invoke($null, [object[]]@(,[string[]]@()))
$updatedSettings.AutoStartMonitoring = $true
$unattendedResumeNeedsStartupArgument = -not $resumeWithoutPromptMethod.Invoke($null, [object[]]@($updatedSettings))
$unattendedStartupResume = $unattendedResumeEnabled -and $unattendedResumeNeedsBothSettings -and $unattendedResumeNeedsStartupArgument -and
    $sourceText.Contains('--windows-autostart') -and $sourceText.Contains('TryOfferSessionResume(resumeWithoutPrompt)')
...
$result = [PSCustomObject]@{
    ...
    HomeCloudBackupStatus = [bool]$homeCloudBackupStatus
    ...
    UnattendedStartupResume = $unattendedStartupResume
    ...
    CloudComputerFolder = [bool]$cloudComputerFolder
    OAuthBuiltInClient = $oauthBuiltInClient
    OAuthRequestForms = [bool]$oauthRequestForms
    OAuthLoginOnlyUi = $oauthLoginOnlyUi
    GmailStorageProtected = [bool]$gmailStorageProtected
    GmailSelfRecipientMime = [bool]$gmailSelfRecipientMime
    GmailLargeReportPayload = [bool]$gmailLargeReportPayload
    GmailMachineNameSubject = [bool]$gmailMachineNameSubject
    GmailOAuthRequest = [bool]$gmailOAuthRequest
    GmailFixedSelfRecipientUi = [bool]$gmailFixedSelfRecipientUi
