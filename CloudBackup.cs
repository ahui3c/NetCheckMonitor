...
namespace NetCheck
{
    ...
    internal sealed class CloudBackupManager : IDisposable
    {
        ...
        {
            var request = (HttpWebRequest)WebRequest.Create(url); request.Method = method; request.Timeout = 60000; request.ReadWriteTimeout = 60000; request.UserAgent = "NetCheckMonitor/0.9.18"; request.Headers[HttpRequestHeader.Authorization] = "Bearer " + token;
