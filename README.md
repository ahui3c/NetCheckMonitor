...
- 增加登入 Windows 後自動執行功能，異常重新啟動後可接續未完成的監控測試。
- 可設定程式開啟後自動開始檢測。
- 增加最多三組自訂測試目標網站或 IP。
- 報表增加每日斷線、延遲、網路介面及更多完整測試數據與資訊。
- 系統匣以不同顏色顯示連線狀態：綠色正常、紅色斷線、橘色檢查中、灰色暫停。
- 顯示設備連線方式、使用網卡及 Wi-Fi 訊號，並寫入 CSV、HTML 與 PDF 報表。
- 增加可選用的進階分層連線診斷功能。
- 完善斷線快速複查、長時間追蹤、異常結束與資料恢復等細節。

完整內容請參閱 [0.9.5 更新說明](docs/RELEASE_NOTES_0.9.5.md)。
...
- 監控期間防止 Windows 自動睡眠；按右上角 X 時縮到系統匣，首次操作會提醒應使用右下角「關閉程式」安全結束，提醒只顯示一次。
