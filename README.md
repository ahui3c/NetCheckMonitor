...
- Windows 登入自動啟動與程式啟動後自動監控同時啟用時，未完成工作階段會免確認直接接續。
- 設定畫面縮短並精簡說明，下方功能按鈕由四行整理為三行。
- 清除資料與重製每日報表改為最後一行的小型維護按鈕，功能與警告提示維持不變。

完整內容請參閱 [0.9.14 更新說明](docs/RELEASE_NOTES_0.9.14.md)。
