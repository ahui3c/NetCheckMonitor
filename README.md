...
- 「關於」頁新增一鍵線上更新：自動下載、驗證、替換並重新啟動正式公開版本。
- 更新期間會安全保存監控狀態；新版啟動後自動接續，失敗時保留或回復舊版。
- 更新套件使用 GitHub SHA-256、檔案清單與 ZIP 路徑安全驗證，且不會修改 `NetCheck_Data`。

完整內容請參閱 [0.9.13 更新說明](docs/RELEASE_NOTES_0.9.13.md)。
...
- 「關於」頁提供 GitHub 專案連結及「線上更新」；按一次即可下載、驗證、替換並重新啟動最新正式版本。
