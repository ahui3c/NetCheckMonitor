...
- 修正 Google Drive 登入授權完成後出現 `client_secret is missing` 的問題，並保留 PKCE 與 `drive.file` 最小權限。
- 程式設定改為與執行檔放在一起的可攜式設定檔，舊版本設定會在背景自動移轉一次。
- 設定頁新增「匯出全部紀錄備份 ZIP」，可將 CSV 原始資料及 HTML／PDF 報表打包下載。

完整內容請參閱 [0.9.7 更新說明](docs/RELEASE_NOTES_0.9.7.md)。
