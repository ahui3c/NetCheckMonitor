...
- Gmail 測試郵件、恢復通知與每日報表標題加入電腦名稱，方便辨識多台設備。
- Google Drive 備份改存入 `Net_Check/電腦名稱`，同一帳戶可清楚整理多台電腦資料。
- 啟用定時測速且當日有資料時，Drive 與 Gmail 日報會一併包含測速 HTML 報表與原始 CSV。

完整內容請參閱 [0.9.11 更新說明](docs/RELEASE_NOTES_0.9.11.md)。
